...
def  read_pdb_chain(pdbname,chain_inf):
  Atomline=[]
  HLposition={}
  ResinameHL={}
  ...
  for line in open(pdbname):
     ...
     if id == 'ATOM' and tem_B !='B':
        Atomline.append(line)
        type = list_n[2]
        if type == 'CA'  and list_n[3]!= 'UNK'  and line[21:22] in list(chain_inf) :
            residue = list_n[3]
...
for line in arr_inf:
    ...
    HLposition, ResinameHL, AtomlineHL = read_pdb_chain(line.strip(),chain_antibody)
    chain_antigen=tem_arr[0][4:5]
    Eposition, ResinameE,AtomlineE= read_pdb_chain(line.strip(),chain_antigen)
