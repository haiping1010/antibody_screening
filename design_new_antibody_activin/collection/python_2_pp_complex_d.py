import sys
import numpy as np
from sklearn.cluster import KMeans
import numpy as np
from sklearn.cluster import KMeans
def one_hot(i):
    m = np.zeros(20, 'uint8')
    m[i] = 1
    return m

def MapResToOnehot(residue1,residue2):
   code={}
   seq=("ALA","ARG", "ASN", "ASP", "CYS", "GLU", "GLN", "GLY", "HIS", "ILE", "LEU", "LYS", "MET", "PHE", "PRO", "SER", "THR", "TRP", "TYR", "VAL")
   index=0
   for name in seq:
              #print (name)
              code[name]=one_hot(index)
              index=index+1
   return (np.concatenate((code[residue1],code[residue2])))
#print (MapResToOnehot("ARG","VAL"))


      
HLposition={}
Eposition={}
ResinameHL={}
ResinameE={}
residuePair=[]
#if len(sys.argv) <1 :
#   print("python python2.py xxx.pdb")
#file=sys.argv[1]
#filebase=file
import glob

def  read_pdb_chain(pdbname,chains):
  Atomline=[]
  HLposition={}
  ResinameHL={}

  for line in open(pdbname):
     #Atomline.append(line)
     tem_B=' '
     if len(line)>16:
        tem_B=line[16]
        line=line[:16]+' '+line[17:]
     #print(line)
     list_n = line.split()
     id = list_n[0]
     if id == 'ATOM' and tem_B !='B':
        Atomline.append(line)
        type = list_n[2]
        if type == 'CA'  and list_n[3]!= 'UNK' and  line[21:22] in list(chains)  :
            residue = list_n[3]
            type_of_chain = line[21:22]
            ##tem1=list_n[5].replace("A", "")
            tem2=line[22:27]
            #tem1=line[22:27].replace("A", "")
            #tem2=tem1.replace("B", "")
            #tem2=tem2.replace("C", "")

            #tem2=filter(str.isdigit, list_n[5])
            atom_count = tem2+line[21:22]
            list_n[6]=line[30:38]
            list_n[7]=line[38:46]
            list_n[8]=line[46:54]
            position = list_n[6:9]
            HLposition[atom_count]=position
            list_n[4]=line[21:22]
            list_n[5]=line[22:26].replace(' ','')
            ResinameHL[atom_count]=residue+list_n[5]+list_n[4]
  return   HLposition, ResinameHL, Atomline



def  caculate_pair(HLposition,Eposition,ResinameHL,ResinameE):
    residuePair=[]
    for key1, value1 in HLposition.items():
       #print (ResinameHL[key1], 'corresponds to', value1)
       for key2, value2 in Eposition.items():
            #print key2,value2
            #print (ResinameE[key2], 'corresponds to', value2)
            ##distance=pow(value1[0]-value2[0])
            a = np.array(value1)
            a1 = a.astype(np.float)
            b = np.array(value2)
            b1 = b.astype(np.float)
            xx=np.subtract(a1,b1) 
            tem=np.square(xx)
            tem1=np.sum(tem)
            out=np.sqrt(tem1)
            #print a
            if out<12 :
                #print (ResinameHL[key1],ResinameHL[key2])
                #print (a,b,out)
                #print (a1)              
                residuePair.append([ResinameHL[key1],ResinameE[key2]])
                #print (antiface)              
    return   residuePair            


import sys

input_f=sys.argv[1]
fr_inf=open(input_f,"r")
arr_inf=fr_inf.readlines()

#print (input_f)
fw=open(input_f+"_contact.txt","w")
for line in arr_inf:
    tem_arr=line.split('-')
    tem_arr_2=tem_arr[0].split('_')
    antig=tem_arr_2[0]
    anti=tem_arr_2[1]
    #chain_E=antig[6:]
    chain_E='A'
    if len(anti[4:])==2:
         chain_HL='HL'
    elif  len(anti[4:])==1:
         chain_HL='H'
    #chain_HL=anti[4:]
    #print (chain_E, chain_HL)
    HLposition, ResinameHL, AtomlineHL = read_pdb_chain(line.strip(),chain_HL)
    Eposition, ResinameE,AtomlineE= read_pdb_chain(line.strip(),chain_E)

    residuePair=caculate_pair(HLposition,Eposition,ResinameHL,ResinameE)
    
    foo = open(line.strip().replace(".pdb","_antibody_poc.pdb"), "w")
    fw_n =open(line.strip().replace(".pdb","_antibody.pdb"), "w")
    length=len(residuePair)
    pdbid=antig+"-"+anti
    fw.write(pdbid+","+","+ str(length))
    fw.write('\n')
    for  value1 in AtomlineHL:
        fw_n.write(value1)
        for value2 in residuePair:
           list_n2 = value1.split()
           #print (value1)
           list_n2[4]=value1[21:22]
           list_n2[5]=value1[22:26].replace(' ','')
           resnameId=list_n2[3]+list_n2[5]+list_n2[4]
           #print value2, resnameId
           if  value2[0] == resnameId:
              #print (value2[0])
              foo.write(value1 )
              break
    foo.close()
    fw_n.close()

    foo = open(line.strip().replace(".pdb","_antigen_poc.pdb"), "w")
    fw_n =open(line.strip().replace(".pdb","_antigen.pdb"), "w")

    for  value1 in AtomlineE:
        fw_n.write(value1)
        for value2 in residuePair:
           list_n2 = value1.split()
           list_n2[4]=value1[21:22]
           list_n2[5]=value1[22:26].replace(' ','')
           resnameId=list_n2[3]+list_n2[5]+list_n2[4]
           #print value2, resnameId
           #print ( value2[1])
           if  value2[1] == resnameId:
              #print (value2[1])
              foo.write(value1 )
              break
    foo.close()
    fw_n.close()



  
fw.close()
