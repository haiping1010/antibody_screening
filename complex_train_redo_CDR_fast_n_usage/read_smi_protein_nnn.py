...
def seq_cat(prot):
    ...
    for i, ch in enumerate(prot[:max_seq_len]):
        if ch in aa_dict_s:
            ...
            x[i] = aa_dict_s['X']  # 使用未知氨基酸的向量
    return x

#seq_voc = "ABCDEFGHIKLMNOPQRSTUVWXYZ"
seq_voc = "ACDEFGHIKLMNPQRSTVWYX"
aa_dict['UNK']=np.zeros(30)
max_seq_len = 95
...
##_CDR.txt
file_path = 'all_screen/' + input_f + '.txt' # 替换成你的文件路径
...
arr_name=[]
'''
arr_name=df['name'].values
'''
for name in arr_frr:
   print (name,list(sequences_dict.keys())[1])
   arr_tem=name.split()
   if   arr_tem[1].replace('.pdb','_CDR.txt')  in   sequences_dict.keys():
       arr_name.append(name.strip())
'''
print (arr_name)
antigen_poc='xxxx_antigen_poc.pdb'
...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, pocket1_graph, y, pocket2_graph):
        ...
        for i in range(data_len):
            ...
            #fr=open(arr_f[0],'r')
            #print (linearr[0])
            c_size, features, edge_index =  pdb_graph(antigen_poc)
            ...
            t=sequences_dict[filename]
            XT=seq_cat(t)
            GCNData.target = torch.tensor(XT, dtype=torch.float32).reshape(95, 30)
...
train_data = TestbedDataset(root='data1', dataset='L_P_'+input_f.replace('_extracted_combined_sequences',''), xd=arr_name, pocket1_graph=pocket1_graph, y=train_Y, pocket2_graph=pocket2_graph)
